...
const Hero = () => {
  return (
    <div className="relative min-h-[80vh] md:min-h-[90vh] flex items-center overflow-hidden pt-20 md:pt-24 md:pb-6">
...
const ServiceCard: React.FC<{ service: any; index: number }> = ({ service, index }) => {
  ...
  const getServiceImage = (serviceId: string) => {
    const images: Record<string, string> = {
      ...
      'data-networking': 'https://i.pinimg.com/1200x/d5/f5/3d/d5f53dcf4814a962ade5df367c935cd5.jpg',
  ...
  return (
    ...
    >
      ...
      <div className="p-8 flex flex-col h-full">
        ...
        <p className="text-sm text-slate-500 leading-relaxed font-medium opacity-80 group-hover:opacity-100 transition-opacity mb-4 flex-grow">
          {service.description}
        </p>
        
        <div className="space-y-2 pt-0 border-t border-slate-50">
          {service.items.slice(0, 3).map((item, idx) => (
            <div key={idx} className="flex items-center gap-2 text-[11px] font-bold text-slate-400 uppercase tracking-tight">
